...
import numpy as np
import rosenpy.model.ptrbfnnc as mynn
...
nn = mynn.PTRBFNN(gpu_enable=False)

# Add layers to the network
nn.add_layer(layer_type="Fully", ishape=input_data.shape[1], neurons=20,
             weights_initializer=init_func.opt_ptrbf_weights,
             bias_initializer=init_func.zeros,
             gamma_initializer=init_func.opt_ptrbf_gamma,
             sigma_initializer=init_func.ones,
             lr_decay_method=decay_func.time_based_decay,
             gamma_rate=1e-2,
             sigma_rate=1e-2)

nn.add_layer(layer_type="Fully", neurons=20, oshape=output_data.shape[1],
             weights_initializer=init_func.opt_ptrbf_weights,
             bias_initializer=init_func.zeros,
             gamma_initializer=init_func.opt_ptrbf_gamma,
             sigma_initializer=init_func.ones,
...
# Train the neural network using the fit method
